...
async def export_following(page, username, csv_path="following.csv", max_following=1000):
    following = []
    seen_ids = set()

    async def handle_response(response):
        if "friends/following/list.json" in response.url and response.status == 200:
            try:
                data = await response.json()
                for user in data.get("users", []):
                    if user["id_str"] not in seen_ids:
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["id_str", "screen_name", "name"])
        for user in following[:max_following]:
            writer.writerow([user["id_str"], user["screen_name"], user["name"]])